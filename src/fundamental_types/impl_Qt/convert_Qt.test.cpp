...
TEST(fundamental_types_convert_Qt, from_qreal)
{
    ...
    EXPECT_TRUE(ul::almost_equal(0.0, from_qreal<double>(qreal{0.0})));
    EXPECT_TRUE(ul::math::approx_equal(0.0L, from_qreal<long double>(qreal{0.0}), 1e-15L));
    EXPECT_TRUE(ul::math::approx_equal(0.0f, from_qreal<float>(qreal{0.0}), 1e-4f));
...
TEST(fundamental_types_convert_Qt, to_qreal)
{
    ...
    EXPECT_TRUE(ul::almost_equal(qreal{0.0}, to_qreal(0.0)));
    EXPECT_TRUE(ul::math::approx_equal(qreal{0.0}, to_qreal(0.0L), qreal{1e-15}));
    EXPECT_TRUE(ul::math::approx_equal(qreal{0.0}, to_qreal(0.0f), qreal{1e-4}));
